...
export const api = {
  ...
  /**
   * Every entry of the calling user that starts inside the window, behind the
   * daily and weekly totals. A page holds 100, so a full week may need paging;
   * Kimai answers 404 rather than an empty list past the last page, hence the
   * catch - whatever was collected so far is still the right answer.
   */
  range: async (s, from, to) => {
    const collected = [];
    for (let page = 1; page <= 3; page += 1) {
      const query = `begin=${encodeURIComponent(localStamp(from))}`
        + `&end=${encodeURIComponent(localStamp(to))}&size=100&page=${page}`;
      let batch;
      try {
        batch = await request(s, `/api/timesheets?${query}`);
      } catch (error) {
        if (page === 1) {
          throw error;
        }
